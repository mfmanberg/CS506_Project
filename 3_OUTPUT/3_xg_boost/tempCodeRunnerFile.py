...
def load_station_data(paths, station_label):
    dfs = []
    for p in paths:
        try:
            df = pd.read_parquet(p)

            # we only keep files that have both Time Stamp + Load
            if {'Time Stamp', 'Load'}.issubset(df.columns):
                dfs.append(df[['Time Stamp', 'Load']])
        except Exception as e:
            print(f"Warning reading {p}: {e}")

    if not dfs:
        print("No valid parquet files found with ['Time Stamp', 'Load'] columns.")
        return pd.DataFrame()
...
def create_aggregates(df):
    if df.empty:
        raise ValueError("create_aggregates got an empty DataFrame (no Load data).")

    if 'Load' not in df.columns:
        raise KeyError(f"'Load' column not found in df. Columns available: {df.columns.tolist()}")
...
def run_xgboost(df, lags=[1, 7, 30], agg_name=''):
    ...
    print(f"\nTrain: {len(train)}, Val: {len(val)}, Test: {len(test)} for {agg_name}")
    ...
    print(f"MAPE: {mape:.2f}% | MAE: {mae:.2f} | RMSE: {rmse:.2f} | R2: {r2:.3f} | Time: {elapsed:.2f}s")

    plt.figure(figsize=(14, 6))
    plt.plot(test['Time Stamp'], y_test, label='Actual', alpha=0.6)
    plt.plot(test['Time Stamp'], y_pred, label='Predicted', alpha=0.8)
    plt.title(f"NYISO Load Prediction for {agg_name}")
    plt.xlabel("Date")
    plt.ylabel("Load (MW)")
    plt.legend()
    plt.show()
